...
async function syncdb() {
  ...
  _0x5447f8.ev.on("connection.update", async _0x3fd0b3 => {
    ...
    if (_0x2224a7 === "open") {
      ...
      let _0x1f88ec = 
`${Config.botname} Activated
Prefix  : ${Config.HANDLERS}
Plugins : ${events.commands.length}
Mode    : ${Config.WORKTYPE}
Database:  ${(isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "Rahmanmd Default")}
\tOther Variables
...
<div class="bounce-text">YOUR RAHMAN MD IS SUCESSFULLY DEPLOYED</body>
</html>
...
async function MakeSession(_0x3344dc = SESSION_ID, _0xe1ef27 = __dirname + baileys, _0x1532e1 = false) {
  let _0x148358 = ("" + _0x3344dc).replace(/^SESSION_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^SESSION_ID_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^wasi_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/Rahman;;;/gi, "").replace(/rahman;;;/gi, "").replace(/RAHMAN;;;/gi, "").trim();
